...
@torch.no_grad()
def group_mixup(X: torch.Tensor, Y: torch.Tensor, Y_bias: torch.Tensor,
        p: torch.Tensor, tau=0.):
    rare = Y != Y_bias
    majority = Y == Y_bias

    X_rare, Y_rare, p_rare = X[rare], Y[rare], p[rare]
    lamb = torch.rand_like(Y_rare, dtype=X.dtype) * tau + (1 - tau)
    lamb = lamb.view(-1, *[1 for _ in range(len(X_rare.shape[1:]))])
    idx = Categorical(probs=(Y_rare[:, None] == Y[None, :]).float()).sample()

    mixed_X_rare = lamb * X_rare + (1 - lamb) * X[idx]
    mixed_Y_rare = Y_rare
    mixed_p_rare = lamb.view(-1, 1) * p_rare + (1 - lamb.view(-1, 1)) * p[idx]

    X = torch.cat([X[majority], mixed_X_rare])
    Y = torch.cat([Y[majority], mixed_Y_rare])
    p = torch.cat([p[majority], mixed_p_rare])
...
class LogitCorrection(pl.LightningModule):
    BIAS_IDX = 0
    TARGET_IDX = -1
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Training set here to estimate confusion matrix
        ...
        assert backbone in LogitCorrection._BACKBONES
        ...
        self.register_buffer('confusion_mat_estimate',
            torch.zeros(self.confusion_mat_shape.numpy().tolist()),
            persistent=True)
    ...
    def add_model_specific_args(parent_parser: ArgumentParser) -> ArgumentParser:
        ...
        # backbone args
        BACKBONES = LogitCorrection._BACKBONES
        ...
        parser.add_argument("--prior_ema", type=float, default=0.5)
        parser.add_argument("--use_group_mixup", type=eval, default=False, choices=[True, False])

        parser.add_argument("--batch_size", type=int, default=128)
        parser.add_argument(
            "--optimizer", choices=LogitCorrection._OPTIMIZERS.keys(), type=str, required=True
        )
        ...
        parser.add_argument(
            "--scheduler", choices=LogitCorrection._SCHEDULERS, type=str, default="none"
        )
    ...
    def configure_optimizers(self) -> Tuple[List, List]:
        ...
        learnable_params = [
            ...
            {
                "name": "biased_classifier",
                "params": filter(lambda p: p.requires_grad, self.biased_classifier.parameters()),
                "weight_decay":  self.weight_decay,
            },
        ]
        ...
        if "warmup" in self.scheduler:
            ...
            max_epochs = self.total_max_epochs - self.current_epoch
            max_scheduler_steps = (
    ...
    def compute_confusion_matrix(self, pred_y, y, a):
    ...
    def forward(self, X: torch.tensor, Y: Optional[torch.tensor] = None) -> Dict[str, Any]:
        ...
        # get biased predictions/ bias attribute estimate
        Z = biased_logits.argmax(-1)

        # update prior estimate
        with torch.no_grad():
            curr_est = self.compute_confusion_matrix(biased_probs, Y, Z).detach()
            self.confusion_mat_estimate.mul_(self.prior_ema).add_((1 - self.prior_ema) * curr_est)

        p = self.confusion_mat_estimate[Z]
        ...
        if self.use_group_mixup:
            ...
            tau = 1 - (math.cos(math.pi * self.trainer.global_step / max_steps) + 1) / 2
            X, Y, p = group_mixup(X, Y, Z, p, tau=tau)
    ...
    def classify_(self, task_idx, logit, target, num_classes,
            gce: float = 0.0, prefix=None):
        if gce > 0.0:
            # use generalized cross entropy
            probs = F.softmax(logit, dim=-1)
            B = torch.arange(target.size(0), device=logit.device)
            Yg = probs[B, target]
            loss_weight = gce * Yg.squeeze().detach()**gce
            loss = loss_weight * F.cross_entropy(logit, target, reduction='none')
            loss = loss.mean()
        else:
        ...
        metrics = {f"class_loss_{task_idx}": loss, f"acc1_{task_idx}": acc1, f"acc5_{task_idx}": acc5, "acc1": acc1}
        if prefix:
    ...
    def get_metrics(self,
        ...
        loss, metrics = self.classify_(
            self.target_variable, logits, targets, self.task_classes, **kwargs)
        out.update(metrics)
        out['loss'] = loss
    ...
    def training_step(self, batch: List[torch.Tensor], batch_idx: int) -> torch.Tensor:
        ...
        outs = self(X, Y)
        logits = outs['logits']
        biased_logits = outs['biased_logits']

        out = self.get_metrics(logits, outs['Y'])
        out_biased = self.get_metrics(biased_logits, Y, gce=self.gce_q, prefix='biased')

        loss = out['loss'] + out_biased['loss']
        out.update(out_biased)
        out['loss'] = loss
    ...
    def validation_step(
        ...
        # Prepare data for bias-unsupervised balanced validation
        if biased_preds is not None:
    ...
    def validation_epoch_end(self, outs: List[Dict[str, Any]]):
        ...
        for i, outs_ in enumerate(outs):  # For all validation datasets
            ...
            u_balanced_acc, u_worst_acc = bias_unsuper_balanced_accuracy(
                unbiased_correct_preds, biased_preds, targets[:, self.TARGET_IDX],
                num_classes=self.confusion_mat_shape)
